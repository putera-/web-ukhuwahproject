<template>
    <footer class="bg-gray-800 text-white">
        <ClientOnly>
            <div v-if="Client.client"
                class="w-full max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-4 p-5 md:p-10">
                <!-- SLOT 1 -->
                <div class="flex flex-col gap-2 items-center">
                    ...
                    <template v-if="Client.client.logo">
                        <img v-if="isURL(Client.client.logo)" :src="Client.client.logo"
                            :alt="Client.client.name + 'Logo'" class="w-40">
                        <img v-else :src="apiUri + Client.client.logo" :alt="Client.client.name + 'Logo'" class="w-40">
                    </template>
